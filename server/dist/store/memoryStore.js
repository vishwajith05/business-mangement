...
const DEFAULT_PASSWORD_HASH = bcryptjs_1.default.hashSync('admin123', 10);
const PARTNER1_HASH = bcryptjs_1.default.hashSync('partner123', 10);
const PARTNER2_HASH = bcryptjs_1.default.hashSync('partner223', 10);
const PARTNER3_HASH = bcryptjs_1.default.hashSync('partner323', 10);
class MemoryStore {
    users = [
        {
            id: 'u-admin-01',
    ...
    async syncInventoryToExcelOnDisk() {
        try {
            ...
            const filePath = path_1.default.join(exportsDir, 'inventory_sync.xlsx');
    ...
    createPartner(partnerData, actor) {
        ...
        const newPartner = {
            ...
            role: role,
            passwordHash: bcryptjs_1.default.hashSync(partnerData.password, 10),
    ...
    updateUser(id, updates, actor) {
        ...
        if (updates.canEditStock !== undefined)
            user.canEditStock = updates.canEditStock;
        if (updates.partnerRegion !== undefined)
            user.partnerRegion = updates.partnerRegion;
        if (updates.phone !== undefined)
            user.phone = updates.phone;
        if (updates.status !== undefined)
            user.status = updates.status;
        if (updates.password) {
            user.passwordHash = bcryptjs_1.default.hashSync(updates.password, 10);
        }
        this.logAudit(actor, 'ACCOUNT_UPDATE', `Updated user account ${user.name} (${user.email}).`);
        const { passwordHash, ...res } = user;
        return res;
    }
    updatePartnerPermissions(id, canEditStock, actor) {
        return this.updateUser(id, { canEditStock }, actor);
    }
    deletePartner(id, actor) {
        const idx = this.users.findIndex(u => u.id === id);
        if (idx === -1)
            return false;
    ...
    updateProduct(id, productData, actor) {
    ...
    // --- Inventory & Stock ---
    adjustStock(productId, quantityChange, changeType, reason, actor) {
    ...
    // --- Sales & Transactions ---
    createSale(partner, customerName, customerEmail, customerPhone, items, paymentMethod = 'Cash') {
        if (items.length === 0) {
    ...
    getSales(actor, filterPartnerId) {
        if (actor.role !== 'ADMIN') {
